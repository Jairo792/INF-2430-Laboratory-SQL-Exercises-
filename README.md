...
```
 SELECT prod.product_name, prod.product_id, prod.aisle_id, prod.department_id dept.department, aisles.aisle, SUM(op_prior.reordered) AS prior_reorders,SUM(op_curr.reordered) AS current_reorders
  FROM
      ic_products AS prod
  JOIN
